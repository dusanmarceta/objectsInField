...
def number_density(n0, rm, d_ref=1000, d=[], alpha=[]):
    ...
    return n, n_total # number density values that corresponds to critical diameters

def ISO_diameters(n, d, alpha, total_number):
    D_s=[] # diameters
...
def total_number(rm, n0, v_min, v_max,
                         u_Sun, v_Sun, w_Sun,
...
def synthetic_population(rm, n0, v_min, v_max,
    ...
    if total_number > maximum_array_size:
        raise Exception("The number of ISOs (N={}) is greater than the defined maximum array size. Try decreasing number-density and/or radius of the model sphere.". format(total_number))
