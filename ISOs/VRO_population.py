...
#sigma_vz=1.6e4
#vd=np.deg2rad(7)
#file_name='M_50_100000.ssm'

#sigma_vx=2.6e4
#sigma_vy=1.8e4
#sigma_vz=1.5e4
#vd=np.deg2rad(12)
#file_name='G_50_100000.ssm'
...
for alpha in alpha: # SFD
    ...
    for run in range(number_of_runs):
        ...
        print('\n ------------------------------ \n Run number {} out of {} for alpha={}'.format(run+1, number_of_runs, alpha))
    ...
    for i in tqdm(range(len(q_out))):
        ...
        iso_moid=moid(1.99330267, -0.1965350, 0, 0.01671022, 1., omega_out[i], Omega_out[i], inc_out[i], e_out[i], q_out[i]/(1-e_out[i]), np.deg2rad(1.))
        
        # writing file for VRO simulation
        np.savetxt(file, np.column_stack([ISO_name, 'COM', np.round(q_out[i],6), np.round(e_out[i],6), np.round(np.rad2deg(inc_out[i]),3), 
                          np.round(np.rad2deg(Omega_out[i])), np.round(np.rad2deg(omega_out[i])), np.round(tp,6), np.round(H,3), 54466.0, 1, 6, np.round(iso_moid[0],6), 'MOPS']), 
                          fmt='%s')
