...
import sys
import numpy as np
from synthetic_population import synthetic_population, total_number, speed_distribution
...
v_divided = np.linspace(v_min, v_max, division_v)
d_divided = np.linspace(min(d), max(d), division_d)

for population in range(1, 3):  # for 3 populations
    ...
    for alpha in Alpha:  # SFD
        ...
        output_file = stars + '_' + str(alpha[0]) + '.ssm'
        np.savetxt(output_file, np.column_stack(np.array([1, 2, 3, 4, 5])))
        ...
        """
        hc_max = np.zeros(division_d - 1)
        for k in range(1, division_d):
            hc_max[k - 1] = max_hc_distance_asteroid(d_divided[k], albedo, V_cut)  # This is OK
        ...
        """
        rm = np.zeros([division_d - 1, division_v - 1])
        for k in range(len(hc_max)):
            rm[k] = hc_max[k] + year2sec(time_of_simulation) * v_divided[1:] / au  # This is OK

        for k1 in range(np.shape(rm)[0]):
            for k2 in range(np.shape(rm)[1]):

                ISO_total_number = total_number(rm=rm[k1][k2], n0=n0, v_min=v_divided[k2], v_max=v_divided[k2 + 1],
                                                ...
                                                d_ref=d_reff, d=[d_divided[k1], d_divided[k1 + 1]],
                                                alpha=alpha)  # This is OK
                ...
                for run in range(number_of_runs):
                    ...
                    np.savetxt('progress_asteroids.txt',
                               ['Run number {} out of {} for SFD={}, population {}, v={}, D={}.'.format(
                                   run + 1, number_of_runs, alpha[0], population, [v_divided[k2], v_divided[k2 + 1]],
                                   [d_divided[k1], d_divided[k1 + 1]])], fmt='%s')

                    print(
                        '\n ------------------------------ \n Run number {} out of {} for SFD={}, population {}, v={}, D={}.'.format(
                            run + 1, number_of_runs, alpha[0], population, [v_divided[k2], v_divided[k2 + 1]],
                            [d_divided[k1], d_divided[k1 + 1]]))

                    print('total number=', ISO_total_number)
                    q, e, f, inc, Omega, omega, D = synthetic_population(rm=rm[k1][k2], n0=n0 / number_of_runs,
                                                                         v_min=v_divided[k2],
                                                                         v_max=v_divided[k2 + 1],
                                                                         u_Sun=u_Sun, v_Sun=v_Sun, w_Sun=w_Sun,
                                                                         sigma_vx=sigma_vx, sigma_vy=sigma_vy,
                                                                         sigma_vz=sigma_vz,
                                                                         vd=vd, va=va, R_reff=R_reff,
                                                                         speed_resolution=100, angle_resolution=90,
                                                                         dr=0.1,
                                                                         d_ref=d_reff,
                                                                         d=[d_divided[k1], d_divided[k1 + 1]],
                                                                         alpha=alpha)  # This is OK
                    # =============================================================================
